import Link from 'next/link'
import Image from 'next/image'

interface ProjectCardProps {
  id: number
  name: string
  image: string
  text: string
  description?: string
  demo_link: string
  github_link: string
  status: string
  featured?: boolean
  tech_stack: string[]
  category: string
}

export default function ProjectCard({
  name,
  image,
  text,
  demo_link,
  github_link,
  status,
  featured = false,
  tech_stack,
  category,
}: ProjectCardProps) {
  return (
    <article className="card bg-base-100 shadow-lg hover:shadow-2xl transition-all duration-300 hover:scale-[1.02] h-full flex flex-col">
      {/* Project Image */}
      <figure className="relative h-48 bg-base-200 overflow-hidden">
        {demo_link ? (
          <Link
            href={demo_link}
            {...(demo_link.startsWith('http')
              ? { target: '_blank', rel: 'noopener noreferrer' }
              : {})}
            className="w-full h-full"
          >
            <Image
              src={image}
              alt={`${name} project screenshot`}
              fill
              className="object-cover hover:scale-110 transition-transform duration-300"
              unoptimized
            />
          </Link>
        ) : (
          <Image
            src={image}
            alt={`${name} project screenshot`}
            fill
            className="object-cover"
            unoptimized
          />
        )}
      </figure>

      <div className="card-body p-6 flex flex-col flex-grow">
        {/* Badges */}
        <div className="flex flex-wrap gap-2 mb-3">
          {featured && <div className="badge badge-primary">Featured</div>}
          <div className="badge badge-outline">{category}</div>
          {status === 'Active' && (
            <div className="badge badge-success badge-outline">
              <div className="w-2 h-2 bg-success rounded-full mr-1 animate-pulse"></div>
              {status}
            </div>
          )}
          {status === 'In Development' && (
            <div className="badge badge-warning badge-outline">{status}</div>
          )}
          {status === 'Archived' && (
            <div className="badge badge-ghost">{status}</div>
          )}
        </div>

        {/* Project Name */}
        <h3 className="card-title text-xl mb-3 line-clamp-2">{name}</h3>

        {/* Description */}
        <p className="text-base-content/70 text-sm leading-relaxed mb-4 line-clamp-3 flex-grow">
          {text}
        </p>

        {/* Tech Stack */}
        <div className="mb-4">
          <div className="flex flex-wrap gap-2">
            {tech_stack.slice(0, 4).map((tech, index) => (
              <span
                key={index}
                className="badge badge-ghost badge-sm text-xs"
              >
                {tech}
              </span>
            ))}
            {tech_stack.length > 4 && (
              <span className="badge badge-ghost badge-sm text-xs">
                +{tech_stack.length - 4} more
              </span>
            )}
          </div>
        </div>

        {/* Action Buttons */}
        <div className="card-actions justify-end gap-2 mt-auto">
          {demo_link && (
            <Link
              href={demo_link}
              {...(demo_link.startsWith('http')
                ? { target: '_blank', rel: 'noopener noreferrer' }
                : {})}
              className="btn btn-primary btn-sm"
            >
              <svg
                className="w-4 h-4"
                fill="none"
                stroke="currentColor"
                viewBox="0 0 24 24"
              >
                <path
                  strokeLinecap="round"
                  strokeLinejoin="round"
                  strokeWidth={2}
                  d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14"
                />
              </svg>
              <span className="hidden sm:inline ml-1">Demo</span>
            </Link>
          )}
          {github_link && (
            <a
              href={github_link}
              className="btn btn-outline btn-sm"
              target="_blank"
              rel="nofollow noopener noreferrer"
            >
              <svg
                className="w-4 h-4"
                fill="currentColor"
                viewBox="0 0 24 24"
              >
                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
              </svg>
              <span className="hidden sm:inline ml-1">Code</span>
            </a>
          )}
        </div>
      </div>
    </article>
  )
}
